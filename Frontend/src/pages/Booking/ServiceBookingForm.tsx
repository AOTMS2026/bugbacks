...
import { CheckCircle2, ArrowRight, ArrowLeft, Briefcase, MapPin, Calendar, Users, Mail, Phone, User as UserIcon } from "lucide-react"
...
export function ServiceBookingForm() {
  ...
  const serviceType = searchParams.get("type") || "General Service"
      ...
      <main className="flex-grow flex items-center justify-center px-4 pt-32 pb-20">
        ...
        {isSuccess ? (
          ...
          >
            ...
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-3">Request Received!</h2>
            <p className="text-gray-500 dark:text-gray-400 mb-8 text-sm leading-relaxed">
              Your inquiry for <strong>{serviceType}</strong> has been successfully sent. Our travel experts will contact you shortly.
            </p>
            <button
            ...
            <div className="px-8 pt-8 pb-6 bg-gray-50/50 dark:bg-white/[0.02] border-b border-gray-100 dark:border-white/5">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h1 className="text-lg font-bold text-gray-900 dark:text-white leading-tight">{serviceType}</h1>
              ...
              <AnimatePresence mode="wait" custom={1}>
                {step === 1 && (
                  <motion.div key="step1" custom={1} variants={slideVariants} initial="hidden" animate="visible" exit="exit" className="absolute inset-0 p-8 flex flex-col justify-center">
                    <div className="mb-6">
                      <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Contact Details</h2>
                    ...
                    <div className="space-y-4">
                      <div className="relative">
                        ...
                        <input autoFocus type="text" name="fullName" value={formData.fullName} onChange={handleChange} className="w-full bg-gray-50 dark:bg-black border border-gray-200 dark:border-white/10 rounded-2xl pl-12 pr-4 py-3.5 focus:ring-2 focus:ring-indigo-500 outline-none text-black dark:text-white transition-all font-medium" />
                      </div>
                      <div className="relative">
                        ...
                        <input type="email" name="email" value={formData.email} onChange={handleChange} className="w-full bg-gray-50 dark:bg-black border border-gray-200 dark:border-white/10 rounded-2xl pl-12 pr-4 py-3.5 focus:ring-2 focus:ring-indigo-500 outline-none text-black dark:text-white transition-all font-medium" />
                      </div>
                      <div className="relative">
                        ...
                        <input type="tel" name="phoneNumber" value={formData.phoneNumber} onChange={handleChange} onKeyDown={(e) => e.key === 'Enter' && nextStep()} className="w-full bg-gray-50 dark:bg-black border border-gray-200 dark:border-white/10 rounded-2xl pl-12 pr-4 py-3.5 focus:ring-2 focus:ring-indigo-500 outline-none text-black dark:text-white transition-all font-medium" />
                      </div>
                    </div>
                  </motion.div>
                )}
                ...
                {step === 2 && (
                  <motion.div key="step2" custom={1} variants={slideVariants} initial="hidden" animate="visible" exit="exit" className="absolute inset-0 p-8 flex flex-col justify-center">
                    <div className="mb-6">
                      <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Service Specifics</h2>
                      <p className="text-sm text-gray-500 dark:text-gray-400">Tell us a bit more about your requirements.</p>
                    </div>
                    <div className="space-y-4">
                      <div className="relative">
                        <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1 block flex items-center gap-2">Destination <span className="text-[10px] bg-gray-200 dark:bg-white/10 px-2 py-0.5 rounded-full lowercase">Optional</span></label>
                        <div className="absolute top-8 left-0 pl-4 flex items-center pointer-events-none">
                          <MapPin className="h-5 w-5 text-gray-400" />
                        </div>
                        <input type="text" name="destination" value={formData.destination} onChange={handleChange} className="w-full bg-gray-50 dark:bg-black border border-gray-200 dark:border-white/10 rounded-2xl pl-12 pr-4 py-3.5 focus:ring-2 focus:ring-indigo-500 outline-none text-black dark:text-white transition-all font-medium" />
                      </div>
                      <div className="relative">
                        <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1 block flex items-center gap-2">Travel Dates <span className="text-[10px] bg-gray-200 dark:bg-white/10 px-2 py-0.5 rounded-full lowercase">Optional</span></label>
                        <div className="absolute top-8 left-0 pl-4 flex items-center pointer-events-none">
                          <Calendar className="h-5 w-5 text-gray-400" />
                        </div>
                        <input type="text" name="travelDates" placeholder="e.g. Next Month" value={formData.travelDates} onChange={handleChange} className="w-full bg-gray-50 dark:bg-black border border-gray-200 dark:border-white/10 rounded-2xl pl-12 pr-4 py-3.5 focus:ring-2 focus:ring-indigo-500 outline-none text-black dark:text-white transition-all font-medium" />
                      </div>
                    </div>
                  </motion.div>
                )}
                ...
                {step === 3 && (
                  <motion.div key="step3" custom={1} variants={slideVariants} initial="hidden" animate="visible" exit="exit" className="absolute inset-0 p-8 flex flex-col justify-center">
                    <div className="mb-6">
                      <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Final Details</h2>
                      <p className="text-sm text-gray-500 dark:text-gray-400">Any special requirements?</p>
                    </div>
                    
                    <div className="mb-4">
                      <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1 block">Number of Travelers</label>
                      <div className="relative">
                        <Users className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                        <input type="number" min="1" name="numberOfTravelers" value={formData.numberOfTravelers} onChange={handleChange} className="w-full bg-gray-50 dark:bg-black border border-gray-200 dark:border-white/10 rounded-2xl pl-10 pr-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none text-black dark:text-white transition-all font-medium" />
                      </div>
                    </div>

                    <div>
                      <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1 block">Special Requests / Notes</label>
                      <textarea name="specialRequests" value={formData.specialRequests} onChange={handleChange} rows={3} placeholder="Please provide any additional details..." className="w-full bg-gray-50 dark:bg-black border border-gray-200 dark:border-white/10 rounded-2xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none text-black dark:text-white transition-all resize-none font-medium text-sm" />
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
