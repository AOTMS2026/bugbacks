...
import { PearlButton } from "./pearl-button";
  ...
  return (
    ...
    >
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={cn(
                  "absolute left-1/2 top-0 aspect-square overflow-hidden rounded-2xl bg-muted shadow-xl will-change-transform cursor-pointer group",
                  cardClassName,
                )}
              ...
              >
                ...
                {/* Overlay with Book Now button */}
                <div className="absolute inset-0 bg-black/20 group-hover:bg-black/50 transition-colors duration-300 flex flex-col items-center justify-center pointer-events-none">
                  <div className="opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 ease-out transform scale-[0.5] pointer-events-auto">
                    <PearlButton 
                      label="Book Now" 
      ...
      {showCaption && active?.title && (
        ...
        >
          <p className="text-[20px] font-bold tracking-tight text-gray-900 dark:text-white">
            {active.title}
          </p>
          {active.subtitle && (
          ...
          {active.meta && active.meta.length > 0 && (
            <dl className="mt-6 w-full max-w-[280px] text-[13px]">
              {active.meta.map((row) => (
                <div key={row.label} className="flex justify-between py-1 border-b border-gray-100 dark:border-white/10 last:border-0">
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          {slides.map((_, index) => (
            <button
              ...
              onClick={() => goTo(index)}
              className={cn(
                "size-2.5 rounded-full bg-indigo-600 dark:bg-indigo-400 transition-opacity",
